...
\begin{enumerate}
    ...
    \item \textbf{Architectural Optimization}: Allowing variable stage count with uniform sizing (Task 5) achieved 22\% better performance (29.17 ps) compared to the manually-tuned 4-stage design in Task 2 (37.32 ps). Both $N=4$ stages with $f = 4.00$ and $N=5$ stages with $f \approx 3.03$ achieve optimal performance, validating theoretical predictions that suggest an optimal sizing factor near $f \approx 3.6$. Further fine-grained optimization around these configurations found that N=4 with sizing (4, 15, 56) achieves the absolute minimum delay of 29.01 ps.
\end{enumerate}
