# gen_sp.py
# 
# 本脚本用于生成Task3优化版本（H1_pro路径）的SPICE仿真文件
# 
# 关键概念：
...
* About critical path1, In reality, when the levels of A1 to A4 remain low, all 15 NOR3 gates connected to node Xinv12 that are not on critical path 1 will have at least one input at a high level.
* A0 -> Xinv01 -> NA0 -> Xinv02 -> PA0 -> XinvPA0 -> NPA0 -> XNOR3_1 -> word_1
* H1min = 16 * 2 * 256 = 8192
* D = (m+3)*8192**[1/(m+3)] + m + 6 = N8192**(1/N) + N + 3

* Buffer inverters (m=4), INV: SN = SP = CM
Xbuffer_0 word_1 buffer_out_0 vdd gnd INV SN='SN_buffer_0' SP='SP_buffer_0' Lg='20n'
Xbuffer_1 buffer_out_0 buffer_out_1 vdd gnd INV SN='SN_buffer_1' SP='SP_buffer_1' Lg='20n'
Xbuffer_2 buffer_out_1 buffer_out_2 vdd gnd INV SN='SN_buffer_2' SP='SP_buffer_2' Lg='20n'
Xbuffer_3 buffer_out_2 buffer_out_3 vdd gnd INV SN='SN_buffer_3' SP='SP_buffer_3' Lg='20n'
...
def load_parameter_lists():
    ...
    best value:
    XinvPA0 (INV):
        CM_XinvPA0: 3.6228946570556264
        SN_XinvPA0（已取整）: 4
        SP_XinvPA0（已取整）: 4
    
    NAND2:
        CM_NAND2: 3.6228946570556264
        SN_NAND2（已取整）: 5
        SP_NAND2（已取整）: 2

    NOR3:
        CM_NOR3: 0.8203353560076379
        SN_NOR3（已取整）: 1
        SP_NOR3（已取整）: 1

    反相器链: (CM, for INV: SN=SP=CM):
        CM values: [1.4859942891369482, 5.383600770529423, 19.5042184672716, 70.66172887512396]
        Sizes for use: [1, 5, 20, 71]
    '''
    # 1. 配置区域
    ...
    gate_configs = {
        "SN_XinvPA0": ("INV",   [3, 4, 5]),
        "SN_NAND2": ("NAND2", [4, 5, 6]), 
        "SN_NOR3": ("NOR3", [1, 2]),
        "SN_buffer_0": ("INV",   [1, 2]),
        "SN_buffer_1": ("INV",   [4, 5, 6]),
        "SN_buffer_2": ("INV",   [18, 20, 22]),
        "SN_buffer_3": ("INV",   [67, 69, 71, 73, 75])
    }
