...
我们通过计算来验证公式\eqref{eq_电流镜公式}是否正确：

仿真得到的电流值为：$I_{\text{out}} = 9.836\mu A$，因此：
\begin{equation}
    \mu_{\text{P}}C_{\text{ox}} = 2I_{\text{out}}/(V_{\text{GS}}-V_{\text{TH}})^2/8 \approx 1.92 \times 10^{-4} A\cdot V^{-2} 
\end{equation}

因此：

\begin{equation}
    I_{\text{out}} = \frac{2}{\mu_{\text{p}} C_{\text{ox}} (W/L)_{\text{P}}}\frac{1}{R_{\text{S}}^2}(\frac{1}{\sqrt{K}}-1)^2 \approx 3.39 \mu A
\end{equation}


与仿真得到的9.836 $\mu A$差异较大，原因可能是：

理论分析推导电流公式时，没有考虑沟道长度调制效应，因为此时晶体管M3的$V_{\text{DS}}$绝对值较大，如果考虑了沟道长度调制效应，$I_{\text{out}}$可能会有所增加。
...
从仿真结果可以观察到，当$V_{\text{DD}}$从0V变化到1.2V时，输出电流$I_{\text{out}}$逐渐增大。然而从理论公式\eqref{eq_电流镜公式}来看，该电流镜的输出电流$I_{\text{out}}$应该与供电电压无关，只要我们确保所有晶体管都工作在饱和区。但是仿真结果确实输出电流随着供电电压增大而增大。我们从图中看出，即便是在任务1.1给定的直流工作点附近（晶体管工作于饱和区），电流依旧无法维持稳定。
...
根据图\ref{fig:task1_3}以及图\ref{fig:task1_4_temp}可以发现，在三种工艺下，在0~70℃内，电流均随着温度升高而呈线性增大，原因可能是随着温度升高，载流子迁移率会减小（因为晶格散射增强），但是沟道中载流子浓度升高，阈值电压也会随之降低，从而导致电流增大。

\textbf{2. 电压对电流的影响}

在该电流镜中，电流$I_{\text{out}}$本不应该随着供电电压$V_{\text{DD}}$升高而升高，但是图\ref{fig:task1_2}中却显示电流随着电压增大而增大，这可能是由于以下原因造成的：

1. 电流镜的输出阻抗有限，当$V_{\text{DD}}$增大时，输出电流可能会受到影响。
2. 沟道长度调制效应可能会导致$I_{\text{out}}$随着$V_{\text{DD}}$的增大而增大。


\textbf{3. 工艺对电流的影响}

由图\ref{fig:task1_4_temp}和\ref{fig:task1_4_vdd}可以发现：
当$V_{\text{DD}}<0.9V$时，ff工艺角的$I_{\text{out}}$最大，ss工艺角的$I_{\text{out}}$最小。当$V_{\text{DD}} > 0.9V$时，ff工艺角的$I_{\text{out}}$最小，tt工艺角和ss工艺角的$I_{\text{out}}$几乎相同。

此外，在$V_{\text{DD}}=1.2V$情况下，不同温度（0~70℃）下，TT工艺角的输出电流绝对值最大，FF工艺角的输出电流绝对值最小。

低供电电压时，FF工艺角输出电流最大的原因可能是由于FF工艺角的晶体管在低电压下能够保持更好的导通状态，从而提供更大的输出电流。

随着供电电压升高，FF工艺角的输出电流的增大趋势逐渐小于SS工艺角和TT工艺角，可能是高电压使得SS工艺角和TT工艺角的晶体管完全导通，并且这两种工艺角的晶体管宽度更大，饱和电流也更大。
