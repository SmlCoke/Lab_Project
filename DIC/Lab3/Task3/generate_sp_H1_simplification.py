...
def load_parameter_accurate(m):
    ...
    buffer_f = hopt/1/1
    for i in range(1, m):
        buffer_cm.append(buffer_f*buffer_cm[-1])
        buffer_sizes_for_use.append(max(1, int(buffer_cm[-1]+0.5)))

    last_node = [f"buffer_out_{0}"]
    # 对于buffer INV，SN = SP = CM
    instances_lines = [f"Xbuffer_0 word_1 buffer_out_0 vdd gnd INV SN='{buffer_sizes_for_use[0]}' SP='{buffer_sizes_for_use[0]}' Lg='20n'\n"]
    for i in range(1, m):
        instances_lines.append(f"Xbuffer_{i} buffer_out_{i-1} buffer_out_{i} vdd gnd INV SN='{buffer_sizes_for_use[i]}' SP='{buffer_sizes_for_use[i]}' Lg='20n'\n")
    ...
    return (CM_XinvPA0, SN_XinvPA0, SP_XinvPA0,
            ...
            buffer_cm, buffer_sizes_for_use,
...
def write_for_N(m, outdir='.'):
    ...
    # 自动创建目录并保存脚本
    path_dir = Path(outdir) / f'H1_pro_m{m}'
    path_dir.mkdir(parents=True, exist_ok=True)
    path_sp = path_dir / f"task3_H1_pro_m{m}.sp"
    path_log = path_dir / f"task3_H1_pro_m{m}.log"
