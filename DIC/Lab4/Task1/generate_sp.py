...
- A->Sum delay (B Cin: (0,0)⊗(1,1), A 0 to 1)
- B->Sum delay (A Cin: (0,0)⊗(1,1), B 0 to 1)
- Cin->Sum delay (A B: (0,0)⊗(1,1), Cin 0 to 1)
- A->Cout delay (001->101, 010->110)
- B->Cout delay (100->110, 001->011)
- Cin->Cout delay (100->101, 010->011)
"""
...
def generate_voltage_sources(input_signal, initial_state, final_state):
    ...
    for signal, node in [('A', 'Ain'), ('B', 'Bin'), ('Cin', 'CIin')]:
        ...
        if signal == input_signal:
            ...
            sources.append(f"V{signal[0] if signal != 'Cin' else 'CI'} {node} GND PULSE ({v1} {v2} 500p 50p 50p 950p 2n)")
        else:
...
def generate_measure_statements(input_signal, output_signal, initial_state, final_state):
    """
    ...
    init_a_val = initial_state['A']
    init_b_val = initial_state['B']
    init_cin_val = initial_state['Cin']
    S_init = (init_a_val ^ init_b_val) ^ init_cin_val
    Cout_init = (init_a_val & init_b_val) | ((init_a_val | init_b_val) & init_cin_val)

    final_a_val = final_state['A']
    final_b_val = final_state['B']
    final_cin_val = final_state['Cin']
    S_final = (final_a_val ^ final_b_val) ^ final_cin_val
    Cout_final = (final_a_val & final_b_val) | ((final_a_val | final_b_val) & final_cin_val)

    if (output_signal == 'Sum'):
        if (S_init == initial_state[input_signal]):
            # Low to High transition
            measures.append(
                f".measure tran tpLH TRIG V({input_node}) VAL='0.5*SUPPLY' RISE=2 "
                f"TARG V({output_signal}) VAL='0.5*SUPPLY' RISE=2"
            )
            # High to Low transition
            measures.append(
                f".measure tran tpHL TRIG V({input_node}) VAL='0.5*SUPPLY' FALL=2 "
                f"TARG V({output_signal}) VAL='0.5*SUPPLY' FALL=2"
            )
        else:
            # Low to High transition
            measures.append(
                f".measure tran tpLH TRIG V({input_node}) VAL='0.5*SUPPLY' FALL=2 "
                f"TARG V({output_signal}) VAL='0.5*SUPPLY' RISE=2"
            )
            # High to Low transition
            measures.append(
                f".measure tran tpHL TRIG V({input_node}) VAL='0.5*SUPPLY' RISE=2 "
                f"TARG V({output_signal}) VAL='0.5*SUPPLY' FALL=2"
            )
    else :
        if (Cout_init == initial_state[input_signal]):
            # Low to High transition
            measures.append(
                f".measure tran tpLH TRIG V({input_node}) VAL='0.5*SUPPLY' RISE=2 "
                f"TARG V({output_signal}) VAL='0.5*SUPPLY' RISE=2"
...
def generate_delay_testbench(fa_type, input_signal, output_signal, initial_state, final_state, output_dir):
    ...
    static_parts = []
    for sig in ['A', 'B', 'Cin']:
        if sig != input_signal:  # input_signal is used to research delay
    ...
    measure_statements = generate_measure_statements(input_signal, output_signal, initial_state, final_state)
    ...
    filename = f"{fa_type}_delay_tb.sp"
