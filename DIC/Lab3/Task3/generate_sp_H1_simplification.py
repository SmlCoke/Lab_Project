...
def load_parameter_accurate(m):
    ...
    # 计算XinvPA0尺寸
    Xinv12_f = hopt/1/1
    XinvPA0_size = Xinv12_f
    XinvPA0_size_for_use = int(XinvPA0_size + 0.5) if XinvPA0_size + 0.5 > 1 else 1

    # 计算XNOR3尺寸
    XinvPA0_f = hopt/1/16
    XNOR3_size = XinvPA0_f * XinvPA0_size
    ...
    instances_lines = [f"Xbuffer_0 word_1 buffer_out_0 vdd gnd INV size = '{buffer_sizes_for_use[0]}' Lg='20n'\n"]
    for i in range(1,m):
    ...
    return XinvPA0_size, XinvPA0_size_for_use, XNOR3_size, XNOR3_size_for_use, buffer_sizes, buffer_sizes_for_use, instances_lines, measures_lines
...
def write_for_N(m, outdir='.'):
    if (load_parameter_accurate(m) is None):
        ...
        return
    XinvPA0_size, XinvPA0_size_for_use, XNOR3_size, XNOR3_size_for_use, buffer_sizes, buffer_sizes_for_use, inst_lines, measure_lines = load_parameter_accurate(m)
    log_content = f'''
    ...
    # 自动创建目录并保存脚本
    path_dir = Path(outdir) / f'H1_pro_m{m}'   # 目标文件夹，例如 ./N=4
    path_dir.mkdir(parents=True, exist_ok=True)  # ✅ 自动创建（含父目录）
    path_sp = path_dir / f"task3_H1_pro_m{m}.sp"   # 最终文件路径
    path_log = path_dir / f"task3_H1_pro_m{m}.log"   # 日志文件路径
