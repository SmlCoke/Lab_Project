...
def load_parameter_lists():
    ...
    最优值参考：
    - XNAND2_size = 3 (小值，步长±1)
    - XNOR2_size = 4 (小值，步长±1)
    - buffer_0_size = 18 (中等值，步长±1)
    """
    # 最优值中心点
    center = {
        'XNAND2_size': 3,
    ...
    # 格式：参数名: (中心值, 扫描半径, 步长)
    scan_config = {
        'XNAND2_size': (center['XNAND2_size'], 1, 1),      # 3±1, 步长1 -> [2, 3, 4]
        'XNOR2_size': (center['XNOR2_size'], 1, 1),        # 4±1, 步长1 -> [3, 4, 5]
        'buffer_0_size': (center['buffer_0_size'], 4, 2),  # 18±4, 步长2 -> [14, 16, 18, 20, 22]
    }
...
def GenSpiceScripts(outdir='.'):
    ...
    sweepdata_content = [".data sweepdata XNAND2_size XNOR2_size buffer_0_size \n"]
    for i, param_dict in enumerate(parameter_lists, 1):
        content = sweepdata_content.append(f"+ {param_dict['XNAND2_size']} {param_dict['XNOR2_size']} {param_dict['buffer_0_size']}\n")
