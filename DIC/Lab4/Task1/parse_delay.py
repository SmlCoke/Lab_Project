...
def parse_delay_file(sp_file):
    ...
    if len(parts) != 4:
        ...
        print(f"Expected format: InputSignal_InitialState_FinalState_OutputSignal")
        return None
...
def create_summary_dataframe(results):
    ...
    for r in results:
        ...
        if 'tpLH' in r:
            row['tpLH_ps'] = round(r['tpLH'] * SECONDS_TO_PICOSECONDS, 3)
        if 'tpHL' in r:
            row['tpHL_ps'] = round(r['tpHL'] * SECONDS_TO_PICOSECONDS, 3)
        if 'tp' in r:
            row['tp_ps'] = round(r['tp'] * SECONDS_TO_PICOSECONDS, 3)
