...
def load_parameter_accurate(m):
    ...
    # 计算XNOR3尺寸
    XNOR3_size = 512/(q**(m+1))
    XNOR3_size_for_use = int(XNOR3_size + 0.5) if XNOR3_size + 0.5 > 1 else 1  # 四舍五入取整

    # 计算仅A0通过的INV尺寸
    XinvPA0_size = 4*XNOR3_size**(1/2)
    XinvPA0_size_for_use = int(XinvPA0_size + 0.5) if XinvPA0_size + 0.5 > 1 else 1  # 四舍五入取整

    # 逐级计算buffer尺寸
    buffer_size = [q*XNOR3_size/2]
    buffer_size_for_use = [int(size+0.5) for size in buffer_size]

    for i in range(1,m):
        buffer_size.append(q*buffer_size[-1])
        buffer_size_for_use.append(int(buffer_size[-1]+0.5))

    # 强制让计算出来的尺寸不为0
    for index in range(len(buffer_size_for_use)):
        if buffer_size_for_use[index] == 0:
            buffer_size_for_use[index] = 1

    last_node = [f"buffer_out_{0}"]
    instances_lines = [f"Xbuffer_0 word_1 buffer_out_0 vdd gnd INV size = '{buffer_size_for_use[0]}' Lg='20n'\n"]
    for i in range(1,m):
        instances_lines.append(f"Xbuffer_{i} buffer_out_{i-1} buffer_out_{i} vdd gnd INV size='{buffer_size_for_use[i]}' Lg='20n'\n")
    ...
    return XinvPA0_size, XinvPA0_size_for_use, XNOR3_size, XNOR3_size_for_use, buffer_size, buffer_size_for_use, instances_lines, measures_lines
...
def write_for_N(m, outdir='.'):
    if (load_parameter_accurate(m) is None):
        ...
        return
    XinvPA0_size, XinvPA0_size_for_use, XNOR3_size, XNOR3_size_for_use, buffer_size, buffer_size_for_use, inst_lines, measure_lines = load_parameter_accurate(m)
    log_content = f'''
    When num of Inverters is {m}:
    best_q {solve_q(m)}
    XinvPA0_size: {XinvPA0_size}
    XinvPA0_size (for use): {XinvPA0_size_for_use}
    XNOR3_size: {XNOR3_size}
    XNOR3_size (for use): {XNOR3_size_for_use}
    all buffer sizes: {buffer_size}
    all buffer sizes (for use): {buffer_size_for_use}
    '''
    content = TEMPLATE_TOP.format(XinvPA0_size = XinvPA0_size_for_use, XNAND2_size = XinvPA0_size_for_use, XNOR3_size = XNOR3_size_for_use, instance_lines=inst_lines, measure_lines=measure_lines)

    # 自动创建目录并保存脚本
    path_dir = Path(outdir) / f'H1_m{m}'   # 目标文件夹，例如 ./N=4
    path_dir.mkdir(parents=True, exist_ok=True)  # ✅ 自动创建（含父目录）
    path_sp = path_dir / f"task3_H1_m{m}.sp"   # 最终文件路径
    path_log = path_dir / f"task3_H1_m{m}.log"   # 日志文件路径
