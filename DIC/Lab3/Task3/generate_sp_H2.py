# gen_sp.py
# 
# 本脚本用于生成Task3（5×32译码器）H2路径的SPICE仿真文件
# 
# 关键概念：
...
def load_parameter_accurate(m):
    ...
    instances_lines = [f"Xbuffer_0 word_2 buffer_out_0 vdd gnd INV SN='{buffer_sizes_for_use[0]}' SP='{buffer_sizes_for_use[0]}' Lg='20n'\n"]
    for i in range(1, m):
...
def write_for_N(m, outdir='.'):
    ...
    # 自动创建目录并保存脚本
    path_dir = Path(outdir) / f'H2_m{m}'
    path_dir.mkdir(parents=True, exist_ok=True)
    path_sp = path_dir / f"task3_H2_m{m}.sp"
    path_log = path_dir / f"task3_H2_m{m}.log"
