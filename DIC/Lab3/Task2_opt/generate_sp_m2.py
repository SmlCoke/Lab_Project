...
def load_parameter_accurate():
    ...
    best value:
    NAND2:
        CM_NAND2 (输入栅电容倍数): 2.3521580450493476
        SN_NAND2 (NMOS尺寸): 3
        SP_NAND2 (PMOS尺寸): 2

    NOR2:
        CM_NOR2 (输入栅电容倍数): 1.844215822963456
        SN_NOR2 (NMOS尺寸): 1
        SP_NOR2 (PMOS尺寸): 2

    反相器链：
        缓冲反相器数量 m: 2
        缓冲反相器输入栅电容: [5.783849446387728, 27.209056013350217]
        缓冲反相器实际用到的尺寸: [6, 27]
    '''
    # 1. 配置区域
    ...
    gate_configs = {
        ...
        "SN_NOR2": ("NOR2", [1, 2]),
        "SN_buffer_0": ("INV",   [4, 6, 8]),
        "SN_buffer_1": ("INV",   [23, 25, 27, 29, 31])
    }
