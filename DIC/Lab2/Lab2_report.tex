...
The measured FO4 delay of approximately 5.946 ps demonstrates the high-speed capability of 16nm FinFET technology. Several observations from the results:

\begin{itemize}
    \item[1.] The $t_{pHL}$ (5.680 ps) is slightly faster than $t_{pLH}$ (6.212 ps), indicating that the NMOS pull-down network has higher drive strength than the PMOS pull-up network in this technology and sizing.
    \item[2.] The difference between the rising delay and falling delay is about 9\%, which shows that the current process and transistor size parameters are well characterized.
\end{itemize}
...
The simulation results for all six configurations are summarized in Table \ref{tab:task2_results}. Key measurements include individual stage delays ($t_{p1}$ through $t_{p4}$) and total chain delay ($t_{\text{p\_total}}$).
...
\toprule
\textbf{Index} & \textbf{f2} & \textbf{f3} & \textbf{f4} & \textbf{$t_{p1}$ (ps)} & \textbf{$t_{p2}$ (ps)} & \textbf{$t_{p3}$ (ps)} & \textbf{$t_{p4}$ (ps)} & \textbf{$t_{\text{p\_total}}$ (ps)} \\
\midrule
1 & 4 & 16 & 64 & 8.619 & 6.786 & 6.752 & 7.009 & 29.17 \\
2 & 4 & 15 & 60 & 8.607 & 6.548 & 6.644 & 7.271 & 29.07 \\
3 & 4 & 20 & 80 & 8.602 & 7.739 & 7.188 & 6.232 & 29.76 \\
4 & 5 & 16 & 64 & 9.749 & 6.279 & 6.477 & 6.958 & 29.46 \\
5 & 5 & 15 & 60 & 9.753 & 6.071 & 6.385 & 7.228 & 29.44 \\
6 & 5 & 20 & 80 & 9.723 & 7.086 & 6.841 & 6.166 & 29.82 \\
\bottomrule
\end{tabular}
...
The optimal configuration is found to be \textbf{$f_2 = 4, f_3 = 15, f_4 = 60$} with a minimum total delay of \textbf{29.07 ps}.
...
The experimental results reveal interesting insights about inverter chain optimization:

For a 4-stage chain with load factor F = 256, the optimal sizing factor should be $f = F^{1/N} = 256^{1/4} = 4$, suggesting uniform scaling with $f_2 = 4, f_3 = 16, f_4 = 64$. The experimental results show that while $f_2 = 4$ matches the theory, the optimal configuration is $f_2 = 4, f_3 = 15, f_4 = 60$, which deviates slightly from the theoretical 4-16-64 progression by using f3=15 instead of 16, and f4=60 instead of 64.

The classical theory assumes identical stage designs with pure capacitive loading. However, in practical FinFET technology, input capacitance includes both gate and parasitic components, while our theory model neglects parasitic capacitance. Moreover, wire parasitic capacitance becomes significant at small feature sizes, causing drive strength to vary non-linearly with transistor sizing.

\textbf{Stage-by-stage analysis}: 
\begin{itemize}
    \item[1.] Configuration (4, 16, 64): All stages have relatively balanced delays, with total delay of 29.17 ps
    \item[2.] Configuration (4, 15, 60): Achieves 29.07 ps by slightly reducing f3 and f4, which reduces their input capacitance while maintaining sufficient drive strength
    \item[3.] Configuration (4, 20, 80): Stage 2 delay increases to 7.74 ps (compared to 6.55 ps for configuration 2), offsetting gains from other stages
\end{itemize}

The results demonstrate that the theoretical 4× scaling provides an excellent starting point, and fine-tuning around this value can yield small improvements. The optimal deviation from theory (using 15 and 60 instead of 16 and 64) suggests that slightly undersizing the intermediate stages can reduce total delay by minimizing parasitic capacitance effects.
...
\begin{enumerate}
    \item[1. ] The optimal sizing configuration from Task 2: f2 = 4, f3 = 15, f4 = 60
...
\begin{itemize}
    \item[1.] \textbf{Total power}: The measured average power of $-49.68 \mu W$ represents the combined dynamic and static power consumption of the 4-stage inverter chain.
...
\[EDP = \text{Power} \times \text{Delay}^2\]

HSPICE simulation is performed with the following key steps:

\begin{enumerate}
    \item[1. ] Use the optimized chain configuration from Task 2 (f2 = 4, f3 = 15, f4 = 60)
    \item[2. ] Sweep supply voltage from 0.5V to 1.5V in 20mV steps
    \item[3. ] Measure total propagation delay ($t_{\text{p\_total}}$) for each voltage
...
\begin{tabular}{cccc}
\toprule
\textbf{Supply (V)} & \textbf{$t_{\text{p\_total}}$ (ps)} & \textbf{Power ($\mu W$)} & \textbf{EDP ($W \cdot s^2$)} \\
\midrule
0.50 & 44.10 & -20.15 & 3.918×10$^{-26}$ \\
0.56 & 38.24 & -26.10 & 3.815×10$^{-26}$ \\
0.58 & 36.74 & -28.28 & 3.817×10$^{-26}$ \\
0.70 & 30.71 & -43.69 & 4.122×10$^{-26}$ \\
0.80 & 27.74 & -59.64 & 4.590×10$^{-26}$ \\
1.00 & 24.36 & -100.7 & 5.978×10$^{-26}$ \\
1.20 & 22.18 & -611.4 & 30.08×10$^{-26}$ \\
1.50 & 18.47 & -8.298e+6 & 2.830×10$^{-21}$ \\
\bottomrule
\end{tabular}
\caption{Selected EDP measurements at different supply voltages}
\end{table}

The \textbf{optimal supply voltage is 0.56V}, achieving the minimum EDP of \textbf{3.815×10$^{-26} W \cdot s^2$} with $t_{\text{p\_total}}$ = \textbf{38.24 ps} and power = \textbf{-26.10 $\mu W$}.
...
\begin{itemize}
    ...
    \item[2.] \textbf{Power characteristics}: Figure \ref{fig:task4_power} shows that power consumption decreases quadratically with supply voltage, following the relationship $P \propto V_{DD}^2$. At 1.0V, power is approximately 100.7 $\mu W$, while at 0.56V it drops to 26.10 $\mu W$ (a 74\% reduction).
    ...
    \begin{itemize}
        \item Speed: Still reasonable at 38.24 ps
        \item Energy efficiency: Power of 26.10 $\mu W$, providing excellent energy-delay trade-off
    \end{itemize}
...
\begin{tabular}{ccc}
\toprule
\textbf{N (Stages)} & \textbf{$t_{\text{p\_total}}$ (ps)} & \textbf{Optimal f} \\
\midrule
1 & 209.3 & 256.00 \\
2 & 45.86 & 16.00 \\
3 & 31.67 & 6.35 \\
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Task 2 (fixed 4 stages with swept sizing): Best delay = 29.07 ps with (f2=4, f3=15, f4=60)
        \item Task 5 (optimized 4 stages with uniform scaling): Delay = 29.17 ps with (f2=4, f3=16, f4=64)
        \item Task 5 fine-grained optimization: Best delay = 29.01 ps with (f2=4, f3=15, f4=56)
        \item The results show that the manual parameter sweep in Task 2 and the fine-grained optimization in Task 5 both converge to similar optimal configurations around f2=4, f3=15
    \end{itemize}
...
\subsection{Fine-Grained Optimization for N=4 and N=5}

Since both N=4 and N=5 stages achieved the same minimum delay of 29.17 ps with uniform sizing factors, we conducted additional fine-grained parameter sweeps around these optimal points to search for even better configurations. Instead of constraining the sizing factors to follow the geometric progression strictly, we allowed small variations in the intermediate stage sizes.

\textbf{For N=4 stages:}

We swept the sizing factors f2, f3, and f4 around the theoretical values (4, 16, 64), allowing variations in f2 (4, 5), f3 (15, 16, 17), and f4 (56, 58, 60, 62, 64, 66, 68, 70). This resulted in 48 different configurations.

\textbf{For N=5 stages:}

We swept the sizing factors f2, f3, f4, and f5 around the theoretical values (3, 9, 27, 84), allowing variations in f2 (2, 3, 4), f3 (7, 9, 11), f4 (25, 27, 29), and f5 (80, 84, 88). This resulted in 81 different configurations.

\textbf{Optimization Results:}

The detailed simulation results are stored in \texttt{Task5\_op/N=4/task5\_op\_N4.csv} and \texttt{Task5\_op/N=5/task5\_op\_N5.csv}. The optimal configurations found are:

\begin{table}[h]
\centering
\begin{tabular}{ccccccc}
\toprule
\textbf{N} & \textbf{f2} & \textbf{f3} & \textbf{f4} & \textbf{f5} & \textbf{$t_{\text{p\_total}}$ (ps)} \\
\midrule
4 & 4 & 15 & 56 & - & 29.01 \\
5 & 3 & 9 & 27 & 80 & 29.11 \\
\bottomrule
\end{tabular}
\caption{Optimal configurations from fine-grained parameter sweep}
\end{table}

The fine-grained optimization revealed that for N=4, the configuration \textbf{(4, 15, 56)} achieves a slightly better delay of \textbf{29.01 ps}, which is marginally better than the uniform scaling result of 29.17 ps. For N=5, the configuration \textbf{(3, 9, 27, 80)} achieves \textbf{29.11 ps}.

\textbf{Analysis:}

The improvement from 29.17 ps to 29.01 ps (0.55\% improvement) demonstrates that:
\begin{itemize}
    \item The theoretical uniform sizing provides an excellent starting point for optimization
    \item Fine-tuning around the theoretical values can yield marginal improvements
    \item The optimization landscape is relatively flat near the optimum, indicating robustness to sizing variations
    \item Practical design can benefit from such parameter sweeps when maximum performance is critical
\end{itemize}

This refined optimization shows that the N=4 configuration with sizing factors (4, 15, 56) represents the best achievable performance for this inverter chain design problem.
...
\begin{enumerate}
    \item \textbf{FO4 Delay Characterization}: The measured FO4 delay of 5.946 ps demonstrates the high-speed capability of advanced FinFET technology. The slight asymmetry between $t_{pLH}$ (6.212 ps) and $t_{pHL}$ (5.680 ps), with about 9\% difference, reflects inherent mobility differences between NMOS and PMOS devices.

    \item \textbf{Theoretical vs. Practical Optimization}: While classical theory predicts uniform 4× scaling for optimal delay, experimental results show that $f_2=4, f_3=15, f_4=60$ provides superior performance for the fixed 4-stage chain in Task 2. This deviation highlights the importance of parasitic effects, non-ideal scaling, and technology-specific characteristics that simple analytical models may overlook.
    ...
    \item \textbf{Architectural Optimization}: Task 2's manual parameter sweep found the optimal 4-stage configuration (4, 15, 60) with 29.07 ps delay. Task 5's uniform sizing approach with $N=4$ stages ($f = 4.00$) yielded 29.17 ps, while $N=5$ stages ($f \approx 3.03$) also achieved 29.17 ps. This validates theoretical predictions suggesting an optimal sizing factor near $f \approx 3.6$. Further fine-grained optimization in Task 5 found that N=4 with sizing (4, 15, 56) achieves 29.01 ps, representing the absolute minimum delay. The convergence of multiple optimization approaches to similar configurations (f2=4, f3=15) demonstrates the robustness of the optimal design region.
\end{enumerate}
