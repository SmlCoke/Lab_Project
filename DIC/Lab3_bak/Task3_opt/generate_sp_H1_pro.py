...
Xbuffer_0 word_1 buffer_out_0 vdd gnd INV size = 'buffer_0_size' Lg='20n'
Xbuffer_1 buffer_out_0 buffer_out_1 vdd gnd INV size='buffer_1_size' Lg='20n'
Xbuffer_2 buffer_out_1 buffer_out_2 vdd gnd INV size='buffer_2_size' Lg='20n'
Xinv_load buffer_out_2 load_out vdd gnd INV size='256' Lg='20n'
...
def load_parameter_lists():
    ...
    # 格式：参数名: (中心值, 扫描半径, 步长)
    scan_config = {
        'XinvPA0_size': (center['XinvPA0_size'], 1, 1),      # 4±1, 步长1 -> [3, 4, 5]
        'XNAND2_size': (center['XNAND2_size'], 1, 1),      # 4±1, 步长1 -> [3, 4, 5]
        'XNOR3_size': (center['XNOR3_size'], 1, 1),        # 1±1, 步长1 -> [1, 2]
        'buffer_0_size': (center['buffer_0_size'], 1, 1),  # 3±1, 步长1 -> [2, 3, 4]
        'buffer_1_size': (center['buffer_1_size'], 2, 2),  # 13±2, 步长2 -> [11, 13, 15]
        'buffer_2_size': (center['buffer_2_size'], 9, 3),  # 57±9, 步长3 -> [48, 51, 54, 57, 60, 63, 66]
    }
...
def GenSpiceScripts(outdir='.'):
    ...
    sweepdata_content = [".data sweepdata XinvPA0_size XNAND2_size XNOR3_size buffer_0_size buffer_1_size buffer_2_size \n"]
    for i, param_dict in enumerate(parameter_lists, 1):
        content = sweepdata_content.append(f"+ {param_dict['XinvPA0_size']} {param_dict['XNAND2_size']} {param_dict['XNOR3_size']} {param_dict['buffer_0_size']} {param_dict['buffer_1_size']} {param_dict['buffer_2_size']}\n")

    sweepdata_content = "".join(sweepdata_content)
    content = TEMPLATE_TOP.format(sweepdata = sweepdata_content)
    file_name = f"task3_opt_H1_pro.sp"
