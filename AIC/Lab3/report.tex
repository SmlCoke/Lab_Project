...
为留出PVT裕量，并考虑深亚微米工艺的寄生效应，初始设计选择：
\begin{equation}
C_c^{initial} = 3\text{ pF}
\end{equation}

经过仿真迭代优化，在保证相位裕度的前提下，为了提高单位增益带宽和FOM，最终取值为：
\begin{equation}
C_c^{final} = 2.4\text{ pF}
\end{equation}

注：以下计算均使用初始值进行设计分析，实际电路采用优化后的最终值。
...
基于初始设计电流$I_5 \approx 104.7\ \mu\text{A}$和初始补偿电容$C_c^{initial} = 3$ pF：
\begin{equation}
SR^{initial} = \frac{I_5}{C_c^{initial}} = \frac{104.7\ \mu\text{A}}{3\text{ pF}} \approx 34.9\text{ V/}\mu\text{s} > 10\text{ V/}\mu\text{s}
\end{equation}

满足压摆率要求。采用优化后的$C_c^{final} = 2.4$ pF，压摆率裕量更大。
...
\begin{itemize}
    ...
    \item M6的跨导达到3.5 mS（3495.5 $\mu$S），满足稳定性要求
