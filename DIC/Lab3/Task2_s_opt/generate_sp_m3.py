...
Xinv_buffer_2 buffer_out_1 buffer_out_2 vdd gnd INV size = 'buffer_2_size' Lg = '20n'
Xinv_load buffer_out_2 load_out vdd gnd INV size = '128' Lg = '20n'
...
def load_parameter_lists():
    ...
    - XNOR2_size = 1 (小值，步长±1)
    - buffer_0_size = 3 (中等值，步长±1)
    - buffer_1_size = 10 (大值，步长±2或±3)
    - buffer_2_size = 35
    """
    # 最优值中心点
    center = {
    ...
    scan_config = {
        ...
        'XNOR2_size': (center['XNOR2_size'], 1, 1),        # 1±1, 步长1 -> [1, 2]
        'buffer_0_size': (center['buffer_0_size'], 1, 1),  # 3±1, 步长1 -> [2, 3, 4]
        'buffer_1_size': (center['buffer_1_size'], 2, 3),  # 10±2, 步长3 -> [8, 11, 14]
        'buffer_2_size': (center['buffer_2_size'], 7, 5),  # 35±7, 步长5 -> [28, 33, 38]
    }
...
def GenSpiceScripts(outdir='.'):
    ...
    sweepdata_content = [".data sweepdata XNAND2_size XNOR2_size buffer_0_size buffer_1_size buffer_2_size\n"]
    for i, param_dict in enumerate(parameter_lists, 1):
        content = sweepdata_content.append(f"+ {param_dict['XNAND2_size']} {param_dict['XNOR2_size']} {param_dict['buffer_0_size']} {param_dict['buffer_1_size']} {param_dict['buffer_2_size']}\n")
